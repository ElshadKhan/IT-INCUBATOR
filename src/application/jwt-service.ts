...
export const jwtService = {
    ...
    async createRefreshJWT(user: UserAccountDBType, deviceId: string) {
        const token = jwt.sign({userId: user.id, deviceId: deviceId}, settings.REFRESH_JWT_TOKEN_SECRET, {expiresIn: "20sec"} )
        return token
    },
