...
export const userRepository = {
    ...
    async updateEmailConfirmation(id: string) {
        let result = await UserModelClass.updateOne({id: id}, {$set: {'emailConfirmation.isConfirmed': true}})
        return  result.modifiedCount === 1
    },
    async updatePasswordConfirmation(id: string) {
        let result = await UserModelClass.updateOne({id: id}, {$set: {'passwordConfirmation.isConfirmed': true}})
        return  result.modifiedCount === 1
    },
    async updateEmailResendingCode(id: string , code: string) {
        let result = await UserModelClass.updateOne({id: id}, {$set: {'emailConfirmation.confirmationCode': code}})
        return  result.modifiedCount === 1
    },
    async updatePasswordResendingCode(id: string , code: string) {
        let result = await UserModelClass.updateOne({id: id}, {$set: {'passwordConfirmation.confirmationCode': code}})
        return  result.modifiedCount === 1
    },
    async updatePassword(id: string , passwordHash: string) {
        let result = await UserModelClass.updateOne({id: id}, {$set: {"accountData.passwordHash": passwordHash}})
        return  result.modifiedCount === 1
    },
