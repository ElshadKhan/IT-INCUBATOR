...
export const sessionsRepository = {
    ...
    async getAllActiveSessions(userId: string): Promise<SessionType[]> {
        const sessions = await SessionModelClass.find({userId: userId}).projection({
            "_id": 0,
            "ip": 1,
            "title": 1,
            "lastActiveDate": 1.,
    ...
    async updateSessions(userId: string, deviceId: string, lastActiveDate: string) {
        const sessions = await SessionModelClass.updateOne({userId: userId, deviceId: deviceId}, {$set: {lastActiveDate: lastActiveDate}})
    ...
    async deleteSessionsByDeviceId(userId: string, deviceId: string) {
        const result = await SessionModelClass.deleteMany({userId: userId, deviceId: deviceId})
        return  result.deletedCount === 1
    },
    async deleteAllSessionsExceptOne(userId: string, deviceId: string) {
        const result = await SessionModelClass.deleteMany({userId: userId, deviceId: {$ne: deviceId}})
        return result.deletedCount === 1
    }
}